    ...
    <div className="w-full bg-white text-zinc-900 border border-zinc-300 font-sans shadow-xs my-8 overflow-hidden">
      ...
      {activeView === 'synergy' && (
        <div className="p-4 sm:p-6 space-y-6 bg-white">
          ...
          {/* Responsive Supply Chain Layout: Map Canvas + Structured Sidebar */}
          <div className="grid grid-cols-1 lg:grid-cols-12 border border-zinc-300 bg-zinc-50">
            {/* Interactive Canvas Viewport */}
            <div className="lg:col-span-8 relative h-[360px] sm:h-[420px] lg:h-[460px] border-b lg:border-b-0 lg:border-r border-zinc-300 overflow-hidden select-none">
              ...
              <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
                <defs>
                  <linearGradient id="pipeFlowLight3" x1="0%" y1="0%" x2="100%" y2="100%">
                ...
                <line x1="18%" y1="30%" x2="52%" y2="45%" stroke="url(#pipeFlowLight3)" strokeWidth="2" strokeDasharray="6 4" />
                <line x1="22%" y1="68%" x2="48%" y2="78%" stroke="url(#pipeFlowLight3)" strokeWidth="2" strokeDasharray="6 4" />
                <line x1="40%" y1="22%" x2="52%" y2="45%" stroke="url(#pipeFlowLight3)" strokeWidth="2" strokeDasharray="6 4" />
              ...
              <div className="absolute inset-0 pointer-events-none z-20">
                {SUPPLY_NODES.map((node) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`border p-1.5 sm:p-2 font-mono text-xs shadow-md backdrop-blur-xs flex items-center gap-2 ${
                          isSelected
                      ...
                      >
                        <div className="shrink-0">
                          {node.type === 'field_scan' && <Atom className={`w-3.5 h-3.5 ${isSelected ? 'text-amber-400' : 'text-amber-600'}`} />}
                          {node.type === 'village_hub' && <Network className={`w-3.5 h-3.5 ${isSelected ? 'text-sky-400' : 'text-sky-600'}`} />}
                          {node.type === 'reactor_vat' && <Zap className={`w-3.5 h-3.5 ${isSelected ? 'text-emerald-400' : 'text-emerald-600'}`} />}
                        </div>

                        <div>
                          <div className={`font-bold text-[10px] sm:text-[11px] leading-tight flex items-center gap-1 ${isSelected ? 'text-white' : 'text-zinc-950'}`}>
                            <span>{node.name.split('(')[0]}</span>
                          </div>
                          <div className={`text-[9px] flex items-center gap-1.5 ${isSelected ? 'text-zinc-300' : 'text-zinc-500'}`}>
                            <span>{node.tonnage} MT</span>
                            <span>•</span>
                            <span className="text-emerald-600 font-bold">{node.efficiencyPct}%</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {/* Dedicated Logistics Node Telemetry Details Panel (No Absolute Collision) */}
            <div className="lg:col-span-4 bg-white p-4 sm:p-5 flex flex-col justify-between space-y-4">
              {selectedSupplyNode ? (
                <div className="bg-zinc-50 border border-zinc-300 p-4 font-mono text-xs shadow-xs h-full flex flex-col justify-between">
                  <div>
                    ...
                    <div className="text-sm font-black text-zinc-950 mb-3">{selectedSupplyNode.name}</div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px] mb-3">
                      <div className="bg-white border border-zinc-200 p-2">
                        ...
                        <span className="text-zinc-900 font-bold">{selectedSupplyNode.tonnage} MT</span>
                      </div>
                      <div className="bg-white border border-zinc-200 p-2">
                        <span className="text-zinc-500 block text-[10px]">CONVERSION</span>
                        <span className="text-emerald-700 font-bold">{selectedSupplyNode.efficiencyPct}%</span>
                      </div>
                      <div className="bg-white border border-zinc-200 p-2">
                        <span className="text-zinc-500 block text-[10px]">LIQUID YIELD</span>
                        <span className="text-sky-800 font-bold">{(selectedSupplyNode.tonnage * 4200).toLocaleString()} L</span>
                      </div>
                      <div className="bg-white border border-zinc-200 p-2">
                        <span className="text-zinc-500 block text-[10px]">UREA DISPLACED</span>
                        <span className="text-amber-800 font-bold">{(selectedSupplyNode.tonnage * 420).toLocaleString()} kg</span>
                      </div>
                    </div>
                  </div>

                  <p className="text-[10px] text-zinc-500 leading-tight pt-2 border-t border-zinc-200">
                    Node telemetry verified via Gemini Vision taxonomy scanner and IoT fermentation sensors in Kalahandi.
                  </p>
                </div>
              ) : (
                <div className="bg-zinc-50 border border-zinc-300 p-6 text-center font-mono text-xs text-zinc-500">
                  Select a logistics node on the map to inspect telemetry data.
                </div>
              )}
            </div>
          </div>
